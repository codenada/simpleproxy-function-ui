...
{{sandbox_templates_js}}
        ...
        function attachTabs() {
          ...
          function openInboundControlsWarning(targetTab) {
            pendingInboundControlsTab = String(targetTab || '').trim() || 'inbound-transform';
            const modal = el('inbound-controls-warning-modal');
            if (!modal || typeof modal.showModal !== 'function') {
              setActiveTab(pendingInboundControlsTab);
              return;
            }
          ...
          switchTab = setActiveTab;
           btns.forEach((btn) => {
             ...
             btn.addEventListener('click', () => {
               ...
               if (name === 'inbound-transform' && currentTabName !== 'inbound-transform') {
                 openInboundControlsWarning(name);
                 return;
               }
               setActiveTab(name);
             });
        ...
        function handleRuleListClick(kind, evt) {
          ...
          el('inbound-controls-continue-btn')?.addEventListener('click', () => {
            el('inbound-controls-warning-modal')?.close();
            if (switchTab) switchTab(pendingInboundControlsTab || 'inbound-transform');
            pendingInboundControlsTab = '';
          });
          el('inbound-controls-status-btn')?.addEventListener('click', () => {
            el('inbound-controls-warning-modal')?.close();
            pendingInboundControlsTab = '';
